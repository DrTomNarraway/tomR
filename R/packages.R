...
#' @title Git It!
#'
#' @description Try to load a package you got from github, or, install it from github if you do not have it yet. If you do not have devtools the function will first install that.
...
git_it <- function(repo, quiet=F) {
  ...
  devtools.installed <- 'devtools' %in% rownames(installed.packages())
  package.installed <- package %in% rownames(installed.packages())
  ...
  if (!package.installed) {
    ...
    else {if(!quiet){message('\n',package,' is installed and up to date (',current.version,').\n')}}
  }
}
